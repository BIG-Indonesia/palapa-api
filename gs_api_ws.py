...
def modifsld(nmfile):
    sldfile = app.config['UPLOAD_FOLDER'] + nmfile.split('.')[0] + '.sld'
    xmlDoc = xmlParser.parse(sldfile)
    rootElement = xmlDoc.getroot()

    ogcu = ''
    ogcl = ''
    for ogc in rootElement.findall('.//{http://www.opengis.net/ogc}PropertyName'):
        ogcu = ogc.text 
        ogcl= ogc.text.lower()
...
#Pengembangan Upload Palapa by Yudha
def allowed_shp(filename):
    return '.' in filename and filename.rsplit('.',)[1] in set(['shp', 'SHP'])

def allowed_shx(filename):
    return '.' in filename and filename.rsplit('.',)[1] in set(['shx', 'SHX'])

def allowed_dbf(filename):
    return '.' in filename and filename.rsplit('.',)[1] in set(['dbf', 'DBF'])

def allowed_prj(filename):
    return '.' in filename and filename.rsplit('.',)[1] in set(['prj', 'PRJ'])
#END
...
def pindahfolder(source_filename, dest_dir):
...
def populateDB(source_shp, database, kodesimpul):
    ...
    try:
        ...
        print(EPSG)
        ogr2ogr.main(["","-f", "PostgreSQL", pg_conn, shape,"-nln", shape_id,"-nlt","PROMOTE_TO_MULTI", "-a_srs",EPSG,"-lco","PRECISION=no"])
...
def resize_image_modif(image_raw, max_width, max_height, tipe):
    image = ''
    header = ''
    print(image)
    if tipe == 'JPEG':
...
#Pengembangan Upload Palapa By Yudha
@app.route('/api/uploadshp', methods=['POST'])
# @auth.login_required
def uploadshp():
    if request.method == 'POST':
        # check if the post request has the file part
        ...
        if file.filename == '':
            ...
            resp = json.dumps({'RTN': 'ERR', 'MSG': 'No selected file'})
            return Response(resp, status=405, mimetype='application/json')
        if not allowed_shp(file.filename):
            resp = json.dumps({'RTN': 'ERR', 'MSG': 'No SHP file!'})
            return Response(resp, status=405, mimetype='application/json')
        if file and allowed_shp(file.filename):
            filename = secure_filename(file.filename)
            file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
            checkfolder=app.config['UPLOAD_FOLDER'] + filename.split('.')[0]
            if not os.path.exists(checkfolder):
                os.makedirs(app.config['UPLOAD_FOLDER'] + filename.split('.')[0])
                #shutil.copy2("/var/palapa/uploads/"+ filename.split('.')[0] +'.shp', "/var/palapa/uploads/"+ filename.split('.')[0] +'/'+filename.split('.')[0] +'.shp')
                shutil.move("/var/palapa/uploads/"+ filename.split('.')[0] +'.shp', "/var/palapa/uploads/"+ filename.split('.')[0] +'/'+filename.split('.')[0] +'.shp')
            else:
                #shutil.copy2("/var/palapa/uploads/"+ filename.split('.')[0] +'.shp', "/var/palapa/uploads/"+ filename.split('.')[0] +'/'+filename.split('.')[0] +'.shp')
                shutil.move("/var/palapa/uploads/"+ filename.split('.')[0] +'.shp', "/var/palapa/uploads/"+ filename.split('.')[0] +'/'+filename.split('.')[0] +'.shp')
            resp = json.dumps({'RTN': filename.split('.')[0],'MSG': 'Unggah data SHP sukses!'})
            return Response(resp, mimetype='application/json')
            #return Response(resp, status=405, mimetype='application/json')

@app.route('/api/uploadshx', methods=['POST'])
# @auth.login_required
def uploadshx():
    if request.method == 'POST':
        # check if the post request has the file part
        ...
        if file.filename == '':
            ...
            resp = json.dumps({'RTN': 'ERR', 'MSG': 'No selected file'})
            return Response(resp, status=405, mimetype='application/json')
        if not allowed_shx(file.filename):
            resp = json.dumps({'RTN': 'ERR', 'MSG': 'No SHX file!'})
            return Response(resp, status=405, mimetype='application/json')
        if file and allowed_shx(file.filename):
            filename = secure_filename(file.filename)

            #CEK FILE SEBELUMNYA
            #list_of_files = glob.glob('/var/palapa/uploads/*') # * means all if need specific format then *.csv
            #latest_file = max(list_of_files, key=os.path.getctime)
            #comp_file = latest_file.split('.')[0]
            # TEST
            file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
            #CHECK NAMA
            #resp = json.dumps({'MSG': 'NAMA FILE:'+comp_file+'-'+filename.split('.')[0]})
            #return Response(resp, mimetype='application/json')
            #END
            checkfolder=app.config['UPLOAD_FOLDER'] + filename.split('.')[0]
            if not os.path.exists(checkfolder):
                resp = json.dumps({'RTN': 'ERR', 'MSG': 'Upload Same SHP First/Not Compatible SHX!!'})
                os.remove(app.config['UPLOAD_FOLDER'] + filename.split('.')[0] + '.shx')
                return Response(resp, status=405, mimetype='application/json')
            else:
                #shutil.copy2("/var/palapa/uploads/"+ filename.split('.')[0] +'.shx', "/var/palapa/uploads/"+ filename.split('.')[0] +'/'+filename.split('.')[0] +'.shx')
                shutil.move("/var/palapa/uploads/"+ filename.split('.')[0] +'.shx', "/var/palapa/uploads/"+ filename.split('.')[0] +'/'+filename.split('.')[0] +'.shx')
            resp = json.dumps({'RTN': filename.split('.')[0], 'MSG': 'Unggah data SHX sukses!'})
            return Response(resp, mimetype='application/json')
            #return Response(resp, status=405, mimetype='application/json')

@app.route('/api/uploaddbf', methods=['POST'])
# @auth.login_required
def uploaddbf():
    if request.method == 'POST':
        # check if the post request has the file part
        ...
        if file.filename == '':
            ...
            resp = json.dumps({'RTN': 'ERR', 'MSG': 'No selected file'})
            return Response(resp, status=405, mimetype='application/json')
        if not allowed_dbf(file.filename):
            resp = json.dumps({'RTN': 'ERR', 'MSG': 'No DBF file!'})
            return Response(resp, status=405, mimetype='application/json')
        if file and allowed_dbf(file.filename):
            filename = secure_filename(file.filename)
            file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
            checkfolder=app.config['UPLOAD_FOLDER'] + filename.split('.')[0]
            if not os.path.exists(checkfolder):
                resp = json.dumps({'RTN': 'ERR', 'MSG': 'Upload Same SHP First/Not Compatible DBF!!'})
                os.remove(app.config['UPLOAD_FOLDER'] + filename.split('.')[0] + '.dbf')
                return Response(resp, status=405, mimetype='application/json')
            else:
                #shutil.copy2("/var/palapa/uploads/"+ filename.split('.')[0] +'.dbf', "/var/palapa/uploads/"+ filename.split('.')[0] +'/'+filename.split('.')[0] +'.dbf')
                shutil.move("/var/palapa/uploads/"+ filename.split('.')[0] +'.dbf', "/var/palapa/uploads/"+ filename.split('.')[0] +'/'+filename.split('.')[0] +'.dbf')
            resp = json.dumps({'RTN': filename.split('.')[0], 'MSG': 'Unggah data DBF sukses!'})
            return Response(resp, mimetype='application/json')
            #return Response(resp, status=405, mimetype='application/json')

@app.route('/api/uploadprj', methods=['POST'])
# @auth.login_required
def uploadprj():
    if request.method == 'POST':
        # check if the post request has the file part
        ...
        if file.filename == '':
            ...
            resp = json.dumps({'RTN': 'ERR', 'MSG': 'No selected file'})
            return Response(resp, status=405, mimetype='application/json')
        if not allowed_prj(file.filename):
            resp = json.dumps({'RTN': 'ERR', 'MSG': 'No PRJ file!'})
            return Response(resp, status=405, mimetype='application/json')
        if file and allowed_prj(file.filename):
            filename = secure_filename(file.filename)
            file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
            checkfolder=app.config['UPLOAD_FOLDER'] + filename.split('.')[0]
            if not os.path.exists(checkfolder):
                resp = json.dumps({'RTN': 'ERR', 'MSG': 'Upload Same SHP First/Not Compatible PRJ!!'})
                os.remove(app.config['UPLOAD_FOLDER'] + filename.split('.')[0] + '.prj')
                return Response(resp, status=405, mimetype='application/json')
            else:
                #shutil.copy2("/var/palapa/uploads/"+ filename.split('.')[0] +'.prj', "/var/palapa/uploads/"+ filename.split('.')[0] +'/'+filename.split('.')[0] +'.prj')
                shutil.move("/var/palapa/uploads/"+ filename.split('.')[0] +'.prj', "/var/palapa/uploads/"+ filename.split('.')[0] +'/'+filename.split('.')[0] +'.prj')
            #Insert to Basisdata
            for f in app.config['UPLOAD_FOLDER'] + '/' + filename.split('.')[0]:
                path, ext = os.path.splitext(f)
                if ext.isupper():
            ...
            for berkas in os.listdir(app.config['UPLOAD_FOLDER'] + '/' + filename.split('.')[0]):
                if berkas.endswith(".tif") or berkas.endswith(".TIF") or berkas.endswith(".tiff") or berkas.endswith(".TIFF"):
                    ...
                    lid = populate[2]
                    SEPSG = populate[1].split(':')[1]
                    resp = json.dumps({'RTN': filename.split('.')[0], 'MSG': 'Unggah data sukses! Proyeksi terdeteksi sebagai: ' + populate[1], 'EPSG': populate[1], 'SEPSG': SEPSG, 'ID': populate[2], 'TID': populate[2], 'UUID': populate[3], 'TIPE': populate[4], 'USER': user, 'GRUP': grup, 'LID': lid.lower().replace('-','_')})
                    return Response(resp, mimetype='application/json')
                if berkas.endswith(".shp") or berkas.endswith(".SHP"):
                    shapefile = app.config['UPLOAD_FOLDER'] + filename.split('.')[0] + '/' + filename.split('.')[0] + '.shp'
                    print "Shapefile: " + shapefile
                    if os.path.exists(shapefile):
                        ...
                        lid = populate[2]+'-'+populate[3]
                        SEPSG = populate[1].split(':')[1]
                        resp = json.dumps({'RTN': filename.split('.')[0], 'MSG': 'Unggah data PRJ sukses!' + populate[0], 'EPSG': populate[1], 'SEPSG': SEPSG, 'ID': populate[2], 'TID': populate[2], 'UUID': populate[3], 'TIPE': populate[4], 'USER': user, 'GRUP': grup, 'LID': lid.lower().replace('-','_')})
                        #DELETE ALL FILE SHP SHX DBF PRJ DIR
                        try:
                            os.remove(app.config['UPLOAD_FOLDER'] + filename.split('.')[0] +'/'+filename.split('.')[0] + '.shp')
                            os.remove(app.config['UPLOAD_FOLDER'] + filename.split('.')[0] +'/'+filename.split('.')[0] + '.shx')
                            os.remove(app.config['UPLOAD_FOLDER'] + filename.split('.')[0] +'/'+filename.split('.')[0] + '.dbf')
                            os.remove(app.config['UPLOAD_FOLDER'] + filename.split('.')[0] +'/'+filename.split('.')[0] + '.prj')
                            #Remove Direcotry
                            shutil.rmtree(app.config['UPLOAD_FOLDER'] + filename.split('.')[0])
                        except OSError as e:
                        ...
                        resp = json.dumps({'RTN': 'ERR', 'MSG': 'No SHAPE file!'})
                        return Response(resp, status=405, mimetype='application/json')
            #resp = json.dumps({'RTN': filename, 'MSG': 'Unggah data PRJ sukses!'})
            #return Response(resp, mimetype='application/json')
# END OF NEW SCRIPT
...
def return_publishkugi():
    if request.method == 'POST':
        ...
        try:
            ...
            # db.session.commit()
            resp = json.dumps({'RTN': filename, 'MSG': 'Publikasi layer ke GeoServer Sukses'})
        except:
            resp = json.dumps({'RTN': 'ERR', 'MSG': 'POST ERROR'})
        return Response(resp, mimetype='application/json')
...
@app.route('/api/minmetadataarc', methods=['POST'])
def minmetadataarc():
    if request.method == 'POST':
        header = json.loads(urllib2.unquote(request.data).split('=')[1])
        workspace = header['pubdata']['WORKSPACE']
	...
	metalinks = Metalinks(identifier=layer_id)
        metalinks.xml = rendered_xml
	metalinks.workspace = workspace
	metalinks.published = 'N'
        metalinks.metatick = 'Y'
        metalinks.akses = akses
        metalinks.keyword = keyword
        metalinks.downloadable = 'N'
	db.session.add(metalinks)
        db.session.commit()
        msg = json.dumps({'MSG':'Metadata service disimpan!'})
        return Response(msg, mimetype='application/json')
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            try:
                ...
                pass
            resp = json.dumps({'RTN': filename, 'MSG': 'Migrasi dataset Sukses!'})
        except:
...
def setfrontend():
    ...
    try:
        ...
        try:
            ...
            image_1 = resize_image_modif(image_1_raw, 1280, 478, image_1_tipe)
        except:
            image_1 = ''
        print 'IMAGE1'
    except:
    ...
    try:
        ...
        try:
            ...
            image_2 = resize_image_modif(image_1_raw, 1280, 478, image_1_tipe)
        except:
            image_2 = ''
        print 'IMAGE2'
    except:
    ...
    try:
        ...
        try:
            ...
            image_3 = resize_image(image_3_raw, 1280, 478, image_3_tipe)
        except:
            image_3 = ''
        print 'IMAGE3'
    except:
    ...
    try:
        ...
        try:
            ...
            image_4 = resize_image(image_4_raw, 1280, 478, image_4_tipe)
        except:
            image_4 = ''
        print 'IMAGE4'
    except:
